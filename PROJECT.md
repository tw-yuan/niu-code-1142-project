...
課程題庫由老師/助教發布或生成課程測驗時會預設建立為 `approved`，並寫入 `reviewed_by/reviewed_at`；題庫 API 支援依審核狀態、題型、測驗與關鍵字篩選，並提供 batch review endpoint 讓核准、退回、草稿、封存不必逐題 PUT。題庫審核頁仍保留退回、草稿與封存按鈕，提供後續品質管理。
