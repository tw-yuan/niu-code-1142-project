...
- CORS middleware
- 全域 rate limit middleware，預設每 IP 每 60 秒 600 requests
- DB startup 初始化
- health check
- WebSocket `/ws`
...
PPTX/DOCX 轉 PDF 時，`converter.py` 會為每次 LibreOffice 呼叫建立獨立 `UserInstallation` profile，避免多個 Celery worker process 同時呼叫 headless LibreOffice 時共用預設 profile 造成偶發 exit code 1。轉換最多重試 3 次；若仍失敗，錯誤訊息會包含每次嘗試的 command、stdout 與 stderr，前端文件錯誤與後台事件可以直接看到可診斷資訊。
...
1. 驗證文件可存取；若帶 `course_id`，文件必須屬於該課程共享文件。
2. 如果指定 `publish_to_course=true`，使用者必須是 instructor 或 ta；學生仍可使用課程共享教材生成自己的私人測驗。
3. 組 context。
...
前端的主要生成入口使用 `POST /quiz/jobs`，建立 `generation_tasks(kind="quiz")` 後由 Celery worker 執行同一套 LLM 生成與保存流程。頁面會透過 `/generation/tasks`、`/generation/tasks/{task_id}` 與 WebSocket `generation_task` 事件顯示 queued/running/failed/succeeded 狀態與三段式進度：整理教材與題目設定、AI 生成、解析並儲存。因此使用者離開測驗頁後，任務仍會在後端完成，完成後重新進入頁面即可從測驗列表看到結果。`POST /quiz/stream` 保留為相容的串流端點。QuizPage 的文件選擇支援「個人文件 / 課程教材」範圍切換，從 Courses 的整課測驗入口帶入 `course` 與 `docs` query 後，會先載入課程 detail 再限制可選文件。QuizPage 在作答區提供「顯示全部答案 / 隱藏全部答案」切換，使用者可在提交前後快速查看所有題目的正解與解析；已提交或達作答上限時仍會優先顯示 diagnostics。
...
初始心智圖生成的主要入口是 `POST /mindmap/jobs`，同樣透過 `generation_tasks` 與 Celery 保存執行狀態與進度。worker 完成後會把 tree JSON 存為 `LearningArtifact(kind="mindmap_tree")`，並將 tree、markdown content 與 artifact id 寫入 task output。`GET /mindmap` 回傳目前使用者可存取文件與該使用者是否已為各文件生成心智圖，MindmapPage 以橫向檔案瀏覽器呈現文件、ready/處理狀態、個人或課程共享來源、心智圖存在狀態與更新時間；點選文件後下方以全寬心智圖畫布顯示既有心智圖或生成入口。心智圖頁使用 `AppLayout` 的 full-width workspace，避免被一般頁面的 `max-w-7xl` 壓縮。`MindmapCanvas` 在桌面顯示右側節點詳情、來源與階層大綱；因文件清單已移到上方，畫布不再被三欄同時擠壓。窄螢幕會把同一份節點資訊固定放在畫布下方，並保留「節點詳情」抽屜作為快速查看入口。節點展開仍使用 SSE，因為它是開啟頁面後針對單一節點的即時互動操作。
...
- 閃卡「存到筆記」使用相同內容、文件、頁碼、來源類型做 idempotent 防重；前端載入既有 flashcard notes 後會把已存過的卡片按鈕顯示為「已存筆記」並停用，避免重新整理或多開頁面造成重複筆記。
...
筆記用來保存 AI 或學習工具產生的文字，主要由 Chat 回覆、Quiz 題目解析與 Flashcard 單張卡片的「存到筆記」動作建立。前端不再提供大面積手動撰寫入口，避免和一般文字編輯器混淆；使用者可在 Notes 頁依課程/文件/關鍵字篩選、inline 編輯、刪除、批量整理與依文件匯出 Markdown。後端 `POST /notes` 對 `chat`、`summary`、`quiz`、`flashcard` 來源採 idempotent 行為：同一使用者、來源類型、文件、session、頁碼與內容完全相同時回傳既有 note，而不是再建立一筆。
...
- 對話 session
- 來源頁碼
- 來源類型：`chat`、`summary`、`quiz`、`flashcard`、`manual`
...
- `POST /courses/{course_id}/members/batch`
...
Courses 的成員分頁提供多選操作。教師可以一次選取多名非 owner 成員，批量改成 `student`、`ta` 或 `instructor`，也可以批量移除；單筆下拉改角色與單筆移除仍保留。前端呼叫 batch endpoint，後端逐筆套用既有權限規則，因此 owner 保護與 instructor 權限限制仍會生效。
...
- `POST /courses/{course_id}/documents/batch-remove`

只有 ready 狀態且由操作者擁有的文件可以加入課程。加入時可附上 `version_label` 與 `note`，並記錄 `added_by`。課程 detail API 會把 `added_at`、`added_by_username`、`version_label`、`note` 一起回傳給前端，讓教師可以在教材分頁辨識教材來源與版本。加入後課程成員可以在 RAG、摘要、心智圖等功能中讀取該文件。移除文件時使用 soft remove：
...
文件擁有者封存文件時，也會把對應的 course document 標成 inactive。教師/TA 端移除多份教材時使用 batch-remove API，一次完成 soft remove，避免前端逐筆 DELETE 中途失敗。
...
Courses 的任務分頁可依任務名稱/文件/測驗關鍵字、任務類型、發布狀態與學生完成狀態篩選。教師/TA 可在任務列直接展開設定表單，修改標題、說明、類型、關聯文件或測驗、截止時間與 `published/draft/archived` 狀態。學生遇到 `custom` 作業時可以填寫完成內容、連結或備註後提交；`_assignment_completion()` 會把最近一次手動提交的 `response` 回傳到 completion，讓學生重新進入課程頁時能看到已提交內容。
...
教師/TA 可以用這些資訊了解學生學習狀態。Courses 頁的學生進度區支援搜尋學生、依風險篩選、依風險/最近活動/測驗平均/姓名排序；點選學生列後會在同一區塊顯示該學生的對話、測驗、閃卡、筆記、待複習與最近活動明細，讓教師不必離開課程頁就能判斷是否需要介入。
...
| `generation_tasks` | 測驗、心智圖、閃卡等可離頁背景生成任務的 input、output、status、error、progress_current、progress_total、progress_message |
...
GET    /courses/{course_id}/progress
GET    /courses/{course_id}/quizzes
PUT    /courses/{course_id}/quizzes/batch
PUT    /courses/{course_id}/quizzes/{course_quiz_id}
GET    /courses/{course_id}/question-bank
PUT    /courses/{course_id}/question-bank/batch
PUT    /courses/{course_id}/question-bank/{item_id}
GET    /courses/{course_id}/announcements
POST   /courses/{course_id}/announcements
PUT    /courses/{course_id}/announcements/{announcement_id}
DELETE /courses/{course_id}/announcements/{announcement_id}
...
`/generation/tasks?kind=quiz|mindmap|flashcards` 預設回傳目前使用者 queued/running 任務，用於頁面重開後恢復「任務執行中」狀態；也可用 `active_only=false&limit=20` 回傳個人近期任務供 Dashboard 顯示。單一 task endpoint 回傳 `input`、`output`、`error`、`artifact_id`、`progress` 與時間欄位，前端用它在 worker 完成後刷新測驗、心智圖或閃卡列表。Admin 另有 `GET /admin/generation-tasks` 可查看全站近期生成任務與使用者資訊。
...
- `course_announcement`：課程公告，包含 `course_id`、`announcement_id`、`title`
- `course_help_request`：課程求助，包含 `course_id`、`help_request_id`、`title`
- `course_help_update`：求助狀態、指派或留言更新，包含 `course_id`、`help_request_id`、`title`、`event_type`、`status`
- `generation_task`：測驗、心智圖、閃卡背景生成任務狀態，包含 `task_id`、`kind`、`status`、`progress`、`output`、`error`

WebSocket backend 使用 Redis pub/sub 讓不同 process 間可以推播給特定 user。前端 `AppLayout` 會把課程公告、求助建立與求助更新轉成可點擊通知；點擊後會前往 `/courses?course=<course_id>&tab=interaction&announcement=<announcement_id>` 或 `/courses?course=<course_id>&tab=interaction&help=<help_request_id>`。
...
1. 全域 middleware：每 client IP 每 60 秒 600 requests，可用 `GLOBAL_RATE_LIMIT_PER_MINUTE` 調整；`/auth/login` 會略過全域桶，改由登入專用限制處理。
2. 特定 endpoint dependency：
   - login：每帳號識別與 IP 組合每 15 分鐘 10 次，且每 IP 每 15 分鐘 120 次；避免同校園或反向代理出口 IP 下，不同使用者共用同一個登入限制桶。
...
Dashboard 也會讀取 `/generation/tasks?active_only=true` 顯示個人執行中的生成任務狀態，包含測驗、閃卡、心智圖的階段訊息與進度百分比。Admin overview 會讀取 `/admin/generation-tasks`，用同一套 `GenerationTaskList` 顯示全站近期任務與使用者資訊。

User Dashboard 實際查詢時使用 active-only，只顯示 queued/running 生成任務；完成後不再佔版面。Admin overview 保留全站近期任務列表，方便管理者追蹤成功與失敗紀錄。
...
Courses 頁支援 `/courses?course=<course_id>&tab=<tab>` deep link。`tab` 可為 `overview`、`materials`、`tasks`、`interaction`、`people`、`question-bank`。互動分頁另支援 `help=<help_request_id>` 與 `announcement=<announcement_id>` target query；頁面會切到公告/求助分頁、清除會遮住目標求助的篩選、捲動並高亮對應卡片。Dashboard 的未讀公告與待處理求助會直接跳到對應課程的 `interaction` 分頁。課程頁版面採上方課程控制帶加下方全寬內容區：建立/加入課程、課程切換集中在頂部，選定課程後主內容不再被固定側欄壓縮，適合教師處理教材、任務、題庫與求助等大量資訊。

課程教材分頁支援批次加入教師自己的 ready 文件，也支援多選課程 ready 教材與「整課問答 / 整課測驗或出題 / 整課閃卡」入口。這些入口會把 `course` 與 `doc`/`docs` query 帶到 Chat、Quiz、Flashcards。Chat、Quiz、Flashcards、Notes 的文件選擇區都能切換個人文件或課程教材範圍，並提供一鍵全選與清空；Chat 的個人文件範圍只顯示目前使用者自己上傳的 ready 文件，課程共享文件只會在選擇課程範圍時出現。Chat 建立 session 前會以目前文件範圍過濾失效 doc id，課程對話仍限制只能使用該課程教材，若教材已被移除會回 400 並提示重新整理選擇。若使用者正在查看舊 session 時改勾文件、切換課程或模式，ChatPage 會提示範圍不同，送出時自動建立一個套用新範圍的新 session，避免 UI 顯示已勾文件但後端仍使用舊 session doc_ids。Flashcards 可接收多個 `doc_ids` 生成閃卡，Notes 在課程範圍下會列出該課程教材相關筆記。

心智圖有獨立側欄入口 `/mindmap`，使用者可在頁面中直接選擇 ready 文件後生成，不必先從文件詳情頁進入；`/mindmap/{doc_id}` deep link 仍保留。

使用者省力操作目前覆蓋：

- 文件：上傳批次、文件多選、批量刪除可刪除文件、多檔對話、多檔測驗。
- Chat / Quiz / Flashcards / Notes：個人文件與課程教材範圍切換；可用範圍內文件全選與清空。
- Quiz：測驗列表多選與批量刪除 owner 自己建立的測驗。
- Flashcards：閃卡多選與批量刪除。
- Notes：保存內容多選、批量刪除與 inline 編輯；Chat 回覆、Quiz 題目解析、Flashcard 單張卡片可一鍵存成筆記，`source_type` 會記錄為 `chat`、`quiz` 或 `flashcard`。
- Courses：教材批次加入、課程教材多選整課操作、教材批量移除、成員批量改角色/移除、任務篩選與任務列內編輯、課程測驗批量設定、題庫批量核准/退回/封存、求助 ticket 篩選與留言。

課程題庫由老師/助教發布或生成課程測驗時會預設建立為 `approved`，並寫入 `reviewed_by/reviewed_at`；題庫 API 支援依審核狀態、題型、測驗與關鍵字篩選，並提供 batch review endpoint 讓核准、退回、草稿、封存不必逐題 PUT。題庫審核頁同樣提供狀態、題型、測驗與題目關鍵字篩選，批量全選會以目前篩選結果為範圍，避免大型課程題庫需要逐題尋找與操作；單題退回、草稿與封存按鈕仍保留，提供後續品質管理。

學生可從課程對話送出求助。Chat 端會把最近數則對話整理到求助內容，並附上 `session_id`；Courses 的求助佇列會回傳並顯示該 session 最近聊天訊息，讓教師/助教能直接看到學生問題、AI 回覆與上下文。教師/TA 可用狀態、優先度、負責人與關鍵字篩選 ticket，處理時可指派自己、新增公開留言或內部註記，並在結案時留下摘要。
...
- `apiUploadWithProgress()` 使用 XHR + FormData 取得每個檔案的上傳進度
- API error 會保留 HTTP status、JSON `detail`、FastAPI validation array、純文字或 HTML error body 清理後的內容；註冊、上傳、設定等表單會直接顯示這些原因，避免只看到 `Request failed`

DocumentsPage 會把上傳 HTTP 錯誤與背景文件處理錯誤分開呈現。上傳成功後佇列項目會記錄 `document_id`，後續透過 `doc_status` / `doc_ready` WebSocket 與 `/documents` 的 `error_msg` 同步 OCR、轉檔、embedding 階段結果；若 worker 將文件設為 `error`，錯誤會顯示在上傳佇列、文件列表與右側文件詳情。
...
- `useGenerationTask()` 查詢 `/generation/tasks` 恢復 active task
- queued/running 時輪詢 `/generation/tasks/{task_id}`
- 同時監聽 WebSocket `generation_task` 事件，完成後呼叫頁面提供的刷新 callback
- Quiz、Flashcards、Mindmap 頁用它搭配 `GenerationTaskStatus` 顯示可點擊的「任務執行中」面板與階段進度，避免離開頁面後生成結果遺失
...
- PPTX/DOCX 轉換依賴 LibreOffice，Docker image 需要包含該套件；若出現 conversion failed，先查看文件詳情中的 stdout/stderr，再判斷是檔案格式、字型、LibreOffice profile 或 timeout 問題。
