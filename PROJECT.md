...
筆記用來保存 AI 或學習工具產生的文字，主要由 Chat 回覆、Quiz 題目解析與 Flashcard 單張卡片的「存到筆記」動作建立。前端不再提供大面積手動撰寫入口，避免和一般文字編輯器混淆；使用者可在 Notes 頁依課程/文件/關鍵字篩選、inline 編輯、刪除、批量整理與依文件匯出 Markdown。
