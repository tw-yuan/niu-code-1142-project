...
- Notes：筆記多選與批量刪除。
- Courses：教材批次加入、課程教材多選整課操作、教材批量移除、題庫批量核准/退回/封存。
...
學生可從課程對話送出求助。Chat 端會把最近數則對話整理到求助內容，並附上 `session_id`；Courses 的求助佇列會回傳並顯示該 session 最近聊天訊息，讓教師/助教能直接看到學生問題、AI 回覆與上下文，再將求助標記為處理中或結案。
