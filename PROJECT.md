...
課程教材分頁支援批次加入教師自己的 ready 文件，也支援多選課程 ready 教材與「整課問答 / 整課測驗或出題 / 整課閃卡」入口。這些入口會把 `course` 與 `doc`/`docs` query 帶到 Chat、Quiz、Flashcards。Chat 與 Quiz 的文件選擇區也提供一鍵全選與清空；Flashcards 可接收多個 `doc_ids` 生成閃卡。

使用者省力操作目前覆蓋：

- 文件：上傳批次、文件多選、批量刪除可刪除文件、多檔對話、多檔測驗。
- Chat / Quiz / Flashcards：文件範圍全選與清空。
