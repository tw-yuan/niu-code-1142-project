...
課程教材分頁支援批次加入教師自己的 ready 文件，也支援多選課程 ready 教材與「整課問答 / 整課測驗或出題 / 整課閃卡」入口。這些入口會把 `course` 與 `doc`/`docs` query 帶到 Chat、Quiz、Flashcards。Chat、Quiz、Flashcards、Notes 的文件選擇區都能切換個人文件或課程教材範圍，並提供一鍵全選與清空；Flashcards 可接收多個 `doc_ids` 生成閃卡，Notes 在課程範圍下會列出該課程教材相關筆記。
