...
前端的主要生成入口使用 `POST /quiz/jobs`，建立 `generation_tasks(kind="quiz")` 後由 Celery worker 執行同一套 LLM 生成與保存流程。頁面會透過 `/generation/tasks`、`/generation/tasks/{task_id}` 與 WebSocket `generation_task` 事件顯示 queued/running/failed/succeeded 狀態；因此使用者離開測驗頁後，任務仍會在後端完成，完成後重新進入頁面即可從測驗列表看到結果。`POST /quiz/stream` 保留為相容的串流端點。
...
初始心智圖生成的主要入口是 `POST /mindmap/jobs`，同樣透過 `generation_tasks` 與 Celery 保存執行狀態。worker 完成後會把 tree JSON 存為 `LearningArtifact(kind="mindmap_tree")`，並將 tree、markdown content 與 artifact id 寫入 task output。節點展開仍使用 SSE，因為它是開啟頁面後針對單一節點的即時互動操作。
...
前端的 AI 生成入口使用 `POST /flashcards/jobs` 建立 `generation_tasks(kind="flashcards")`。worker 完成後解析 JSON 並建立 `flashcards` rows，頁面收到 `generation_task` succeeded 或輪詢到 succeeded 後重新載入列表。`POST /flashcards/stream` 保留給舊版即時串流流程。
...
| `generation_tasks` | 測驗、心智圖、閃卡等可離頁背景生成任務的 input、output、status、error |
...
`/generation/tasks?kind=quiz|mindmap|flashcards` 回傳目前使用者 queued/running 任務，用於頁面重開後恢復「任務執行中」狀態。單一 task endpoint 回傳 `input`、`output`、`error`、`artifact_id` 與時間欄位，前端用它在 worker 完成後刷新測驗、心智圖或閃卡列表。
...
- `generation_task`：測驗、心智圖、閃卡背景生成任務狀態，包含 `task_id`、`kind`、`status`、`output`、`error`
...
- Quiz、Flashcards、Mindmap 頁用它顯示「任務執行中」，避免離開頁面後生成結果遺失
