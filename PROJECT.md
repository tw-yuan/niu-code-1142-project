...
Courses 頁支援 `/courses?course=<course_id>&tab=<tab>` deep link。`tab` 可為 `overview`、`materials`、`tasks`、`interaction`、`people`、`question-bank`。Dashboard 的未讀公告與待處理求助會直接跳到對應課程的 `interaction` 分頁。課程頁版面採上方課程控制帶加下方全寬內容區：建立/加入課程、課程切換集中在頂部，選定課程後主內容不再被固定側欄壓縮，適合教師處理教材、任務、題庫與求助等大量資訊。
