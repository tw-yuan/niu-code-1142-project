...
interface AdminUserUsage {
  ...
  recent_events: Array<{ id: string; feature: string; model: string; tokens_used: number; created_at: string }>
}

interface AdminUserDetail extends AdminUser {
  usage: AdminUserUsage
  recent_audit_logs: Array<{ id: string; action: string; resource: string | null; created_at: string }>
  deletion: { requested_at: string | null; scheduled_at: string | null; export_expires_at: string | null }
}
...
export function AdminPage() {
  ...
  const selectedFeatureRows = useMemo(
    () => Object.entries(selected?.usage.by_feature ?? {}).map(([feature, tokens]) => ({ feature, tokens })),
    [selected],
  )
  ...
  async function loadAll() {
    setError("")
    const [nextStats, nextCost, nextReliability, logs, config] = await Promise.all([
  ...
  async function loadUsers() {
    ...
    const data = await apiFetch<AdminUserList>(`/admin/users?${params.toString()}`)
    setUsers(data.items)
  ...
  async function forcePurge() {
    if (!selected) return
    const confirmed = window.confirm(`確定要永久清除 ${selected.username} 與所有資料？此操作無法復原。`)
    if (!confirmed) return
    setMessage("")
    setError("")
    await apiFetch(`/admin/users/${selected.id}/force-purge`, { method: "POST" })
    setSelected(null)
    setMessage("使用者資料已清除")
    await loadUsers()
  }
  ...
  function handleError(err: unknown) {
    setError(err instanceof ApiError || err instanceof Error ? err.message : "操作失敗")
  }
    ...
    <div>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button className="inline-flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm hover:bg-zinc-50" onClick={() => loadAll().catch(handleError)}>
            <RefreshCw size={16} />
            重新整理
          </button>
        </div>
      ...
      {adminTab === "users" && createOpen && (
        <section className="mb-6 rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
          ...
          <form className="grid gap-3 md:grid-cols-3 xl:grid-cols-6" onSubmit={(event) => createUser(event).catch(handleError)}>
            <Input label="帳號" value={createForm.username} onChange={(value) => setCreateForm((form) => ({ ...form, username: value }))} />
            <Input label="Email" type="email" value={createForm.email} onChange={(value) => setCreateForm((form) => ({ ...form, email: value }))} />
            <Input label="初始密碼" type="password" value={createForm.password} onChange={(value) => setCreateForm((form) => ({ ...form, password: value }))} />
            <Select label="角色" value={createForm.role} onChange={(value) => setCreateForm((form) => ({ ...form, role: value as "student" | "admin" }))} options={[["student", "學生"], ["admin", "管理員"]]} />
            <NumberInput label="Token 配額" value={createForm.token_quota} onChange={(value) => setCreateForm((form) => ({ ...form, token_quota: value }))} />
      ...
      <div className="mb-6 grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(420px,0.65fr)]">
          ...
          <div className="border-b border-zinc-200 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-zinc-500">{totalUsers} 筆</span>
              </div>
              <div className="grid gap-2 sm:grid-cols-[1fr_130px_130px]">
                <label className="relative">
                  ...
                  <input className="w-full rounded-lg border border-zinc-200 py-2 pl-9 pr-3 text-sm" value={q} onChange={(event) => setQ(event.target.value)} placeholder="搜尋帳號或 Email" />
                </label>
                <select className="rounded-lg border border-zinc-200 px-3 py-2 text-sm" value={role} onChange={(event) => setRole(event.target.value)}>
                  <option value="">全部角色</option>
                  <option value="student">學生</option>
                  <option value="admin">管理員</option>
                </select>
                <select className="rounded-lg border border-zinc-200 px-3 py-2 text-sm" value={active} onChange={(event) => setActive(event.target.value)}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              ...
              <tbody className="divide-y divide-zinc-100">
                {users.map((user) => (
                  <tr key={user.id} className={selected?.id === user.id ? "bg-indigo-50/60" : "hover:bg-zinc-50"}>
                    <td className="px-4 py-3">
                      <button className="text-left" onClick={() => selectUser(user.id).catch(handleError)}>
                        <div className="font-medium text-zinc-900">{user.username}</div>
                        <div className="text-xs text-zinc-500">{user.email}</div>
                      </button>
                    </td>
                    <td className="px-4 py-3">{user.role === "admin" ? "管理員" : "學生"}</td>
                    <td className="px-4 py-3">
                      <StatusBadge active={user.is_active} />
                    </td>
                    <td className="px-4 py-3">{user.document_count} · {formatBytes(user.storage_bytes)}</td>
                    <td className="px-4 py-3">
                      <div>{formatNumber(user.token_used_this_month)} / {formatNumber(user.token_quota)}</div>
                      <div className="mt-1 h-1.5 rounded-full bg-zinc-100">
                        <div className={quotaBarClass(user.quota_status)} style={{ width: `${Math.min(100, user.quota_percent)}%` }} />
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs text-zinc-500">{shortDate(user.created_at)}</td>
                    <td className="px-4 py-3">
                      <button className="inline-flex items-center gap-1 rounded-md border border-zinc-200 px-2 py-1 text-xs hover:bg-zinc-50" onClick={() => toggleActive(user).catch(handleError)}>
        ...
        <section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
          {selected ? (
            <div>
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-zinc-500">{selected.id}</p>
                </div>
                <StatusBadge active={selected.is_active} />
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <Input label="帳號" value={selected.username} onChange={(value) => setSelected((user) => user ? { ...user, username: value } : user)} />
                <Input label="Email" type="email" value={selected.email} onChange={(value) => setSelected((user) => user ? { ...user, email: value } : user)} />
                <Select label="角色" value={selected.role} onChange={(value) => setSelected((user) => user ? { ...user, role: value as "student" | "admin" } : user)} options={[["student", "學生"], ["admin", "管理員"]]} />
                <Select label="狀態" value={String(selected.is_active)} onChange={(value) => setSelected((user) => user ? { ...user, is_active: Number(value) } : user)} options={[["1", "啟用"], ["0", "停用"]]} />
                <NumberInput label="上傳配額 MB" value={selected.quota_mb} onChange={(value) => setSelected((user) => user ? { ...user, quota_mb: value } : user)} />
                <NumberInput label="Token 月配額" value={selected.token_quota} onChange={(value) => setSelected((user) => user ? { ...user, token_quota: value } : user)} />
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                <button className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700" onClick={() => saveSelected().catch(handleError)}>
                  <Save size={16} />
                  儲存
                </button>
                <button className="inline-flex items-center gap-2 rounded-lg border border-red-200 px-3 py-2 text-sm text-red-600 hover:bg-red-50" onClick={() => forcePurge().catch(handleError)}>
                  <Trash2 size={16} />
                  永久清除
                </button>
              </div>
              ...
              <div className="mt-6 border-t border-zinc-200 pt-4">
                ...
                <div className="flex gap-2">
                  <input className="min-w-0 flex-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="至少 8 個字元" />
                  <button className="inline-flex items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 text-sm hover:bg-zinc-50" onClick={() => resetPassword().catch(handleError)} disabled={password.length < 8}>
                    <KeyRound size={16} />
                    重設
                  </button>
                </div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                ...
                <MiniStat label="容量" value={formatBytes(selected.storage_bytes)} />
                <MiniStat label="Token %" value={`${selected.usage.quota_percent}%`} />
              </div>
            ...
            <div className="flex min-h-80 items-center justify-center text-sm text-zinc-500">選擇一位使用者查看詳情</div>
          )}
        </section>
      </div>
      )}
      ...
      <section className="mb-6 rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 flex items-center gap-3">
          <button className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700" onClick={() => saveConfig().catch(handleError)}>
            儲存設定
          </button>
        ...
        <div className="divide-y divide-zinc-100">
          {auditLogs.map((log) => (
            <div key={String(log.id)} className="grid gap-2 px-5 py-3 text-xs sm:grid-cols-[170px_180px_1fr]">
              <div className="text-zinc-500">{shortDate(String(log.created_at))}</div>
...
function Input({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: string }) {
  return (
    <label className="block">
      ...
      <input className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" type={type} value={value} onChange={(event) => onChange(event.target.value)} />
    </label>
  )
}
...
function NumberInput({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-medium text-zinc-500">{label}</span>
      <input className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" type="number" min={1} value={value} onChange={(event) => onChange(Number(event.target.value))} />
    </label>
  )
}
...
function Select({ label, value, options, onChange }: { label: string; value: string; options: [string, string][]; onChange: (value: string) => void }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-medium text-zinc-500">{label}</span>
      <select className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" value={value} onChange={(event) => onChange(event.target.value)}>
...
function StatusBadge({ active }: { active: number }) {
  return (
    <span className={active ? "rounded-full bg-green-50 px-2 py-1 text-xs text-green-700" : "rounded-full bg-zinc-100 px-2 py-1 text-xs text-zinc-500"}>
      {active ? "啟用" : "停用"}
    </span>
  )
}
...
function quotaBarClass(statusValue: AdminUser["quota_status"]) {
  const color = statusValue === "exceeded" ? "bg-red-600" : statusValue === "warning" ? "bg-amber-500" : "bg-indigo-600"
