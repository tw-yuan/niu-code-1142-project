...
export function MindmapCanvas({
  ...
  const parsedTree = useMemo(() => tree ?? markdownToTree(markdown), [tree, markdown])
  ...
  const searchMatches = useMemo(() => findMatches(parsedTree, query), [parsedTree, query])
  const layout = useMemo(
    () => (parsedTree ? layoutTree(parsedTree.root, collapsed, searchMatches.ancestorIds) : null),
    [parsedTree, collapsed, searchMatches.ancestorIds],
  )
  const selected = useMemo(
    () => (parsedTree ? findNode(parsedTree.root, selectedId) ?? parsedTree.root : null),
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
      <div className="flex flex-col gap-2 border-b border-zinc-200 bg-white p-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-zinc-800">階層心智圖</div>
            <div className="text-xs text-zinc-500">預設聚焦骨架，可逐層展開細節</div>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            <Search size={15} className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-zinc-400" />
            <input
          ...
          <IconButton label="展開到第 2 層" onClick={() => setCollapsedDepth(2)}>
            <Layers size={16} />
          </IconButton>
          ...
          <IconButton label="縮小" onClick={() => setZoom((value) => Math.max(MIN_ZOOM, value - 0.1))}>
            <Minus size={16} />
          </IconButton>
          <IconButton label="重設縮放" onClick={() => setZoom(1)}>
            <Maximize2 size={16} />
          </IconButton>
          <IconButton label="放大" onClick={() => setZoom((value) => Math.min(MAX_ZOOM, value + 0.1))}>
            <Plus size={16} />
          </IconButton>
          <button
      ...
      <div className="grid min-h-[640px] lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className={[showOutline ? "hidden lg:block" : "block", "min-w-0 bg-zinc-50"].join(" ")}>
          <div className="max-h-[74vh] overflow-auto p-4">
            ...
            >
              ...
              {layout.nodes.map((node) => {
                ...
                const collapsedNode = collapsed.has(node.id) && hasChildren(node)
                return (
                  <g key={node.id} transform={`translate(${node.x}, ${node.y})`}>
                    <rect
                      ...
                      height={node.height}
                      rx={8}
                      fill={isRoot ? "#4f46e5" : isMatch ? "#eef2ff" : "#ffffff"}
                      stroke={isSelected ? "#4f46e5" : isMatch ? "#818cf8" : "#d4d4d8"}
                    ...
                    {hasChildren(node) && (
                      ...
                      >
                        <circle cx={18} cy={node.height / 2} r={13} fill={isRoot ? "#4338ca" : "#f4f4f5"} />
                        {collapsedNode ? (
                          <ChevronRight x={10} y={node.height / 2 - 8} size={16} color={isRoot ? "#ffffff" : "#52525b"} />
                        ) : (
                          <ChevronDown x={10} y={node.height / 2 - 8} size={16} color={isRoot ? "#ffffff" : "#52525b"} />
                        )}
                      </g>
                    )}
                    <g
                    ...
                    >
                      <foreignObject x={hasChildren(node) ? 36 : 12} y={0} width={node.width - (hasChildren(node) ? 48 : 24)} height={node.height}>
                        <div className={["flex h-full flex-col justify-center", isRoot ? "text-white" : "text-zinc-800"].join(" ")}>
                          <div className="line-clamp-2 text-sm font-semibold leading-5">{node.title}</div>
                          {!isRoot && (
                            <div className={["mt-0.5 flex items-center gap-1 text-[11px]", typeColor(node.type)].join(" ")}>
                              <CircleDot size={10} />
                              <span>{typeLabel(node.type)}</span>
                              {node.hiddenChildCount > 0 && <span className="text-zinc-500">+{node.hiddenChildCount}</span>}
                            </div>
                          )}
                        </div>
                      </foreignObject>
        ...
        <aside className={[showOutline ? "block" : "hidden lg:block", "border-l border-zinc-200 bg-white"].join(" ")}>
          <div className="max-h-[74vh] overflow-auto">
            <div className="border-b border-zinc-200 p-4">
              <div className="text-xs font-medium uppercase tracking-wide text-zinc-400">目前節點</div>
              <h2 className="mt-1 text-base font-semibold text-zinc-900">{selected?.title}</h2>
              {selected?.summary && <p className="mt-2 text-sm leading-6 text-zinc-600">{selected.summary}</p>}
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="rounded-md bg-zinc-100 px-2 py-1 text-xs text-zinc-600">第 {selected?.depth ?? 0} 層</span>
            ...
            {selected?.source_refs && selected.source_refs.length > 0 && (
              <div className="border-b border-zinc-200 p-4">
                <div className="mb-2 text-xs font-medium uppercase tracking-wide text-zinc-400">來源</div>
                <div className="space-y-2">
                  {selected.source_refs.map((ref, index) => (
                    <div key={`${ref.page_num}-${ref.chunk_index}-${index}`} className="rounded-md border border-zinc-200 px-3 py-2 text-xs text-zinc-600">
            ...
            <div className="p-4">
              <div className="mb-3 text-xs font-medium uppercase tracking-wide text-zinc-400">大綱</div>
              <OutlineTree
...
function IconButton({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <button className="rounded-md p-2 text-zinc-600 hover:bg-zinc-100" title={label} aria-label={label} onClick={onClick}>
      {children}
    </button>
  )
}
  ...
  return (
    <div>
      <div className="flex items-center gap-1 py-1">
        ...
        >
          {hasChildren(node) ? collapsed.has(node.id) ? <ChevronRight size={15} /> : <ChevronDown size={15} /> : <span className="h-1.5 w-1.5 rounded-full bg-zinc-300" />}
        </button>
        <button
          className={[
            "min-w-0 flex-1 rounded px-2 py-1 text-left text-sm",
            selectedId === node.id ? "bg-indigo-50 font-medium text-indigo-700" : "text-zinc-700 hover:bg-zinc-50",
          ].join(" ")}
      ...
      {childrenVisible && node.children.length > 0 && (
        <div className="ml-5 border-l border-zinc-200 pl-2">
          {node.children.map((child) => (
            <OutlineTree key={child.id} node={child} selectedId={selectedId} collapsed={collapsed} onSelect={onSelect} onToggle={onToggle} />
          ))}
        </div>
      )}
    </div>
...
function markdownToTree(markdown: string): MindmapTree | null {
  ...
  const stack: Array<{ markdownDepth: number; node: MindmapNode }> = [{ markdownDepth: 0, node: root }]
  let counter = 0
  ...
  for (const raw of markdown.split("\n")) {
    ...
    if (markdownDepth === 1 && root.title === "心智圖" && root.children.length === 0) {
      root.title = label
      continue
    }
    while (stack.length > 1 && stack[stack.length - 1].markdownDepth >= markdownDepth) stack.pop()
...
function layoutTree(root: MindmapNode, collapsed: Set<string>, forcedOpen: Set<string>) {
...
function cleanLabel(value: string) {
  return value.replace(/\*\*/g, "").replace(/^#+\s*/, "").trim().slice(0, 36)
}
