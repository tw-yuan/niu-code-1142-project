          ...
          <div key={f.id}>
            {f.status === 'success' ? (
              <a
                ...
                className="flex items-start gap-2 px-4 py-3 border rounded-lg text-sm text-blue-600 hover:bg-blue-50 transition"
              >
                <Download className="w-4 h-4 shrink-0 mt-0.5" />
                <span className="min-w-0">
                  <span className="block font-medium text-blue-700">{f.title || FORMAT_LABELS[f.format] || f.format}</span>
                  <span className="block text-xs text-gray-500 truncate">
                    {f.filename || FORMAT_LABELS[f.format] || f.format}
                  </span>
                  {f.purpose && <span className="block text-xs text-gray-500 mt-1">{f.purpose}</span>}
                </span>
              </a>
            ) : (
              <div className="flex items-center gap-2 px-4 py-2 border border-red-200 rounded-lg text-sm text-red-500">
                <AlertCircle className="w-4 h-4" />
                <span>{f.title || FORMAT_LABELS[f.format] || f.format} 產生失敗</span>
              </div>
            )}
          </div>
        ))}
