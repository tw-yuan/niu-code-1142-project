...
- 文件管理：支援 PDF、Markdown、PPTX、DOCX，上傳後以 Celery 背景處理，透過 WebSocket 推播狀態；上傳 HTTP 錯誤、OCR/索引背景任務錯誤與後端 `error_msg` 會顯示在上傳佇列、文件列與文件詳情，方便追蹤失敗原因。
- 文件處理 pipeline：檔案轉圖片、Vision OCR、OCR 快取、文字切分、Embedding、ChromaDB upsert；Office 轉 PDF 會使用隔離的 LibreOffice profile 與重試，失敗時保留 stdout/stderr 方便追查。
...
- 學習工具：摘要、測驗生成與作答、心智圖、Anki 式正反面閃卡與間隔複習、錯題本、AI/生成內容保存筆記、學習目標；心智圖有獨立側欄入口，以檔案瀏覽器式橫向清單列出每份可用文件與是否已有心智圖，並以全寬畫布呈現節點；節點文字資訊、來源與階層大綱在桌面右側顯示，窄螢幕固定放在畫布下方並可用抽屜快速查看，避免壓縮圖面；測驗會正規化選項與答案，避免 A/B/C/D 與完整選項文字不一致造成誤判，並可一鍵顯示/隱藏全部答案；測驗、閃卡、筆記可切換個人文件或課程教材範圍，Chat/Quiz/Flashcards 可一鍵存成筆記，其中閃卡存筆記會防止同卡重複建立；測驗、心智圖、閃卡生成會以 Celery 背景任務保存進度，功能頁可點擊查看階段進度，離開頁面後仍會完成並回寫資料庫；Markdown 與心智圖節點文字支援 LaTeX/KaTeX 顯示。
- 課程功能：教師建立課程、加入碼、成員角色與批量管理、文件共享、全寬課程操作頁、教材批次加入與移除、教材版本/備註欄位、多選整課問答/測驗/閃卡、公告、可篩選/編輯/封存的作業、自訂作業提交內容、含聊天脈絡/留言/內部註記/結案摘要的求助 ticket、可點擊並定位到公告/求助的 WebSocket 通知、可單筆或批量編輯設定的課程測驗、可篩選/批量審核的預設核准題庫，以及可搜尋、排序、依風險篩選與查看單一學生明細的學習進度。
...
`LLM_API_KEY` 與 `LLM_BASE_URL` 是三種 LLM 功能的共用 fallback。若要分開控成本，可另外設定 `LLM_CHAT_API_KEY` / `LLM_CHAT_BASE_URL`、`LLM_VISION_API_KEY` / `LLM_VISION_BASE_URL`、`LLM_EMBED_API_KEY` / `LLM_EMBED_BASE_URL`。逾時也可分開控制：`LLM_TIMEOUT_SECONDS` 是共用預設，`LLM_CHAT_TIMEOUT_SECONDS`、`LLM_VISION_TIMEOUT_SECONDS`、`LLM_EMBED_TIMEOUT_SECONDS` 可覆蓋單一功能；OCR 圖片請求建議 vision timeout 至少 120 秒。
