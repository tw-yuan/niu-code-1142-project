...
- RAG 對話：支援文件範圍、課程文件範圍、空白對話快速提問建議、問題改寫、引用來源、SSE streaming 回答；Socratic 模式不會把學生問題插入 system prompt；若使用者在既有對話旁改勾文件範圍，送出時會自動建立套用新範圍的對話；建立對話時會過濾已失效文件並用明確錯誤提示取代模糊 404。
