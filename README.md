...
- RAG 對話：支援文件範圍、課程文件範圍、空白對話快速提問建議、問題改寫、引用來源、SSE streaming 回答。
- 學習工具：摘要、測驗生成與作答、心智圖、閃卡與間隔複習、錯題本、筆記、學習目標；測驗、閃卡、筆記可切換個人文件或課程教材範圍，Chat/Quiz/Flashcards 可一鍵存成筆記；測驗、心智圖、閃卡生成會以 Celery 背景任務保存進度，功能頁可點擊查看階段進度，離開頁面後仍會完成並回寫資料庫；Markdown 與心智圖節點文字支援 LaTeX/KaTeX 顯示。
- 課程功能：教師建立課程、加入碼、成員角色與批量管理、文件共享、全寬課程操作頁、教材批次加入與移除、教材版本/備註欄位、多選整課問答/測驗/閃卡、公告、作業、含聊天脈絡/留言/內部註記/結案摘要的求助 ticket、可單筆或批量編輯設定的課程測驗、可篩選/批量審核的預設核准題庫，以及可搜尋、排序、依風險篩選與查看單一學生明細的學習進度。
