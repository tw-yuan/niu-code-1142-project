...
- RAG 對話：支援文件範圍、課程文件範圍、空白對話快速提問建議、問題改寫、引用來源、SSE streaming 回答；建立對話時會過濾已失效文件並用明確錯誤提示取代模糊 404。
- 學習工具：摘要、測驗生成與作答、心智圖、Anki 式正反面閃卡與間隔複習、錯題本、AI/生成內容保存筆記、學習目標；心智圖有獨立側欄入口並可直接選擇 ready 文件；測驗會正規化選項與答案，避免 A/B/C/D 與完整選項文字不一致造成誤判；測驗、閃卡、筆記可切換個人文件或課程教材範圍，Chat/Quiz/Flashcards 可一鍵存成筆記；測驗、心智圖、閃卡生成會以 Celery 背景任務保存進度，功能頁可點擊查看階段進度，離開頁面後仍會完成並回寫資料庫；Markdown 與心智圖節點文字支援 LaTeX/KaTeX 顯示。
