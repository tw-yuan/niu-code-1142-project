...
一個以 Python FastAPI 為後端、串接 OpenAI-compatible API 的期末專題 Web 系統。使用者可上傳課程資料與作業需求，由 AI 提供講解並自行決定要產生哪些可下載交付檔案。

> **定位**：課程實驗 Demo，小範圍展示 AI 檔案理解、內容生成與文件輸出流程，不會自動送交外部平台。
...
| 管理者登入 | 獨立密碼，進入後台管理頁面 |
| 課程資料上傳 | 可上傳任何檔案格式；常見文件、表格與圖片會嘗試解析（可選） |
| 作業輸入 | 上傳作業檔案或輸入作業敘述，至少提供一項，也可同時提供 |
| AI 決定交付檔案 | 由模型決定要產生 PDF、DOCX、TXT 或 XLSX，以及每份檔案內容 |
| AI 生成 | 串接 OpenAI-compatible API，產生講解與交付檔案內容 |
| 即時進度 | SSE 即時顯示處理階段 |
| 詳細過程 | 可展開查看檔案解析、需求拆解、引用來源等紀錄 |
| 結果頁 | 顯示 AI 講解、交付檔案用途與下載連結 |
| 歷史紀錄 | 查看 / 刪除過去任務與結果 |
| 後台設定 | API endpoint、模型、溫度、提示詞、檔案限制、測試連線 |
| 課程 Demo | 小範圍課程實驗使用，不自動送交外部平台 |
...
3. 進入主頁後：
   - **左側**：上傳課程資料（講義、筆記等，可選）
   - **右側**：上傳作業檔案或輸入作業敘述，至少提供一項；也可以同時提供
   - 可先點一下上傳區，再用 `Ctrl+V` 貼上剪貼簿中的檔案或圖片
4. 點「開始生成」
5. AI 會先提供講解，並自行決定要產生哪些交付檔案
6. 查看即時進度 → 展開詳細過程 → 檢視結果
7. 複製文字或下載 AI 決定產生的檔案
8. 可從右上角進入「歷史紀錄」查看過去的任務

### 作業敘述 Prompt 範本

如果沒有上傳作業檔案，可以在「作業敘述」貼上類似下面的內容。填得越完整，AI 越能產生可修改、可查證的草稿。

```text
作業主題：
請在這裡寫題目或老師要求，例如「分析某篇文章的核心觀點」或「完成一份資料分析報告」。

作業類型：
請說明是報告、閱讀心得、程式作業、計算題、數據分析、簡報大綱或其他類型。

需要完成的內容：
1. 第一個要求，例如摘要、背景說明或問題拆解。
2. 第二個要求，例如提出論點、解題步驟、資料分析或程式邏輯。
3. 第三個要求，例如結論、反思、檢查清單或延伸建議。

格式限制：
請寫字數、段落、引用格式、程式語言、輸出格式、是否需要表格或圖表。

評分重點：
請寫老師特別重視的項目，例如資料引用、分析深度、程式可執行、計算過程或個人觀點。

我希望 AI 協助：
請寫你想要 AI 產生大綱、草稿、解題步驟、檢查清單、程式架構或修改建議。

已知限制：
請寫目前缺少哪些資料、哪些數據要自行確認、哪些內容需要保留自己的想法。
```

若已經有作業題目檔案，可以上傳到「作業檔案上傳」，也可以在作業敘述補充格式、評分重點或希望 AI 協助的方向。左側課程資料仍可上傳作為參考。系統不限制副檔名；PDF、DOCX、TXT、MD、XLSX、CSV 與常見圖片會嘗試解析，其他格式會保留檔名、格式與大小供 AI 參考。
...
| 格式 | 上傳 | 解析方式 | 說明 |
|------|:----:|------|------|
| PDF | ✅ | 擷取文字 | 純圖片 PDF 可能無法解析文字 |
| DOCX | ✅ | 擷取段落與表格 | 可作為作業題目或課程資料 |
| TXT / MD | ✅ | 擷取文字 | 自動偵測編碼 |
| XLSX / CSV | ✅ | 擷取表格摘要 | 取前 100 筆供 AI 參考 |
| PNG / JPG / WEBP | ✅ | 圖片附件 | 送給支援 vision 的模型判讀 |
| 其他格式 | ✅ | Metadata only | 保留檔名、格式與大小，不因格式被拒絕 |
...
如果 PDF 是純圖片掃描檔，系統可能無法擷取文字。可以補充作業敘述，或另外上傳圖片讓支援 vision 的模型判讀。
