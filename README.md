...
- 學習工具：摘要、測驗生成與作答、心智圖、閃卡與間隔複習、錯題本、可編輯筆記、學習目標；心智圖有獨立側欄入口並可直接選擇 ready 文件；測驗會正規化選項與答案，避免 A/B/C/D 與完整選項文字不一致造成誤判；測驗、閃卡、筆記可切換個人文件或課程教材範圍，Chat/Quiz/Flashcards 可一鍵存成筆記；測驗、心智圖、閃卡生成會以 Celery 背景任務保存進度，功能頁可點擊查看階段進度，離開頁面後仍會完成並回寫資料庫；Markdown 與心智圖節點文字支援 LaTeX/KaTeX 顯示。
